...
def chatbot_response(chat_input, placeholder_response):
    ...
    st.session_state.current_conversation_history.append({"role": "user", "content": chat_input})

    # Calculate the total tokens in the conversation history
    total_tokens = sum(count_tokens(message["content"]) for message in st.session_state.current_conversation_history)
    ...
    while total_tokens > MAX_MEMORY_TOKENS:
        if len(st.session_state.current_conversation_history) > 2:
            removed_message = st.session_state.current_conversation_history.pop(1)
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=st.session_state.current_conversation_history,
        stream=True,
    )
    ...
    st.session_state.current_conversation_history.append({"role": "assistant", "content": assistant_response})
